  ...
  rownamesStyle=NULL, showNA=FALSE, characterNA="", byrow=FALSE)
{
  ...
  cellBlock <- new(J("org/cran/rexcel/RCellBlock"), sheet,
           as.integer(startRow-1), as.integer(startColumn-1),
           as.integer(blockRows), as.integer(blockCols),
           TRUE)

  # insert colnames
  if (col.names) {                   
    .jcall( cellBlock, "V", setDataFrameHeaderMethod, 0L, jOffset,
  ...
  for (j in 1:ncol(x)) {
   ...
   .jcall( cellBlock, "V", setDataFrameColumnMethod, as.integer(j+jOffset-1L), iOffset,
...
{
    isXSSF <- .jcall( cellBlock, 'Z', 'isXSSF' )
    .jcall( cellBlock, 'V', 'setFill',
            .Rcolor2XLcolor( fill$foregroundColor, isXSSF ), .Rcolor2XLcolor( fill$backgroundColor, isXSSF ),
